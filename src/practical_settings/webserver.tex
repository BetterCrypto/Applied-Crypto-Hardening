...
\begin{itemize*}
  ...
  \item Apache 2.2.15 linked against OpenSSL 1.0.1e, CentOS 6.5
\end{itemize*}
...
\subsubsection{Tested with Version}
\begin{itemize*}
 \item Apache 2.2.12 linked against OpenSSL 0.9.8j, SLES 11 SP 3
\end{itemize*}

\paragraph{Settings}
Enabled modules \emph{SSL} and \emph{Headers} are required.
\configfile{vhost-ssl.conf.sles}{200-207}{SSL configuration for an Apache vhost}

Ensure that SSL Compression is disabled in \emph{/etc/sysconfig/apache2}, as you cannot use \emph{SSLCompression off} in the VHost configuration:
\configfile{sysconfig-apache2.sles}{288}{SSL Compression Setting in sysconfig}
